 ...
 *
 * `getRepositories()` inspects `process.env.DATABASE_URL` once and decides
 * whether to wire up the Prisma adapters (real DB) or the in-memory mock
 * adapters. Callers should use the same instance across a single
 * request lifecycle to avoid reading / writing two different stores.
 */
